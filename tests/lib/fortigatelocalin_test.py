...
header {
  ...
  target:: fortigatelocalin from-id 2
}
"""
...
header {
  comment:: "this is a test acl"
  target:: fortigatelocalin ngfw-mode policy-based
}
"""
...
header {
  comment:: "this is a test acl"
  target:: fortigatelocalin edge-filter
}
"""
...
class FortigateLocalInTest(unittest.TestCase):
  ...
  def testServices(self):
    ...
    custom_port_acl = fortigatelocalin.FortigateLocalIn(
        policy.ParsePolicy(GOOD_HEADER + custom_port_term, self.naming),
        EXP_INFO,
    )
  ...
  def testInterfaces(self):
    ...
    no_interfaces_sig = 'set intf any'
    src_int_only_sig = 'set intf any'
    dest_int_only_sig = 'set intf wan2'
    both_interfaces_sig = 'set intf wan2'
  ...
  def testExpiration(self):
    ...
    expiration_term = self.fmt.format(
        TERM_TEMPLATE,
        add_fields={'expiration': '2099-12-31', 'comment': '"test expiration"'},
    )
  ...
  def testApplication_ID(self):
    ...
    """
    application_term = textwrap.dedent("""\
        term test-app-id {
          destination-address:: SOME_HOST
          destination-port:: HTTP
          protocol:: tcp
          action:: accept
          fortigate-application-id:: 15816
        }
        """)
  ...
  def testDuplicateTermError(self):
    ...
    self.assertRaises(
        fortigate.FortiGateDuplicateTermError,
        fortigatelocalin.FortigateLocalIn,
        parsed_p,
        EXP_INFO,
    )
  ...
  def testPortMap(self):
    ...
    self.assertRaises(
        fortigate.FortiGatePortDoesNotExistError,
        port_map.get_protocol,
        'tcp',
        5000,
    )
    self.assertRaises(
        fortigate.FortiGateValueError, port_map.get_protocol, 'bad_proto', 22
    )
