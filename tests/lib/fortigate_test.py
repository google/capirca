...
class FortigateTest(unittest.TestCase):
  ...
  def testAction(self):
    ...
    self.assertTrue(
      accept_sig in str(accept_acl), '[%s]' % str(accept_acl)
    )
    self.assertTrue(
      deny_sig in str(deny_sig), '[%s]' % str(deny_acl)
    )
    self.assertTrue(
      deny_sig in str(reject_acl), '[%s]' % str(reject_acl)
    )

  def testAddresses(self):
    diff_addr_term = self.fmt.format(
      TERM_TEMPLATE, src_addr='SOME_HOST',
      dest_addr='SOME_HOST2'
    )
    same_addr_term = self.fmt.format(
      TERM_TEMPLATE, src_addr='SOME_HOST2',
      dest_addr='SOME_HOST2'
    )
    any_src_term = self.fmt.format(
      TERM_TEMPLATE, remove_fields=('src_addr',)
    )
    any_dest_term = self.fmt.format(
      TERM_TEMPLATE, remove_fields=('dest_addr',)
    )
    ...
    self.assertTrue(
      any_src_sig in str(any_src_acl), '[%s]' % str(any_src_acl)
    )
    self.assertTrue(
      any_dest_sig in str(any_dest_acl), '[%s]' % str(any_dest_acl)
    )
  ...
  def testServices(self):
    ...
    ip_term = self.fmt.format(TERM_TEMPLATE,
                              remove_fields=('dest_port', 'src_port', 'protocol'))
    ...
    self.assertTrue(
      dup_sig in str(dup_acl), '[%s]' % str(dup_acl)
    )
    self.assertTrue(
      diff_sig in str(diff_acl), '[%s]' % str(diff_acl)
    )
    self.assertTrue(
      src_only_sig in str(src_only_acl), '[%s]' % str(src_only_acl)
    )
    self.assertTrue(
      icmp_sig in str(icmp_acl), '[%s]' % str(icmp_acl)
    )
    self.assertTrue(
      ip_sig in str(ip_acl), '[%s]' % str(ip_acl)
    )
    self.assertTrue(
      custom_port_sig in str(custom_port_acl), '[%s]' % str(custom_port_acl)
    )
  ...
  def testInterfaces(self):
    ...
    self.assertTrue(
      no_interfaces_sig in str(no_interfaces_acl), '[%s]' % str(no_interfaces_acl)
    )
    self.assertTrue(
      src_int_only_sig in str(src_only_int_acl), '[%s]' % str(src_only_int_acl)
    )
    self.assertTrue(
      dest_int_only_sig in str(dest_only_int_acl), '[%s]' % str(dest_only_int_acl)
    )
    self.assertTrue(
      both_interfaces_sig in str(both_interfaces_acl), '[%s]' % str(both_interfaces_acl)
    )
  ...
  def testLogging(self):
    ...
    self.assertTrue(
      log_sig in str(log_acl), '[%s]' % str(log_acl)
    )
    self.assertTrue(
      log_sig not in str(no_log_term), '[%s]' % str(no_log_acl)
    )
